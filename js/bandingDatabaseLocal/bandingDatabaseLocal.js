var bandingDatabaseLocal = new baseObject2({
	prefix : 'bandingDatabaseLocal',
	url : 'pages.php?page=bandingDatabaseLocal',
	formName : 'bandingDatabaseLocalForm',
	...
	refreshList: function(){
		$.ajax({
			...
			success: function(data) {
				var resp = eval('(' + data + ')');
				if(resp.err==''){
					$("#bandingDatabaseLocalForm").html(resp.content.tableContent);
				}else{
					bandingDatabaseLocal.errorAlert(resp.err);
				}
			}
		});
	},
	...
	Edit: function(){
	  var errMsg = bandingDatabaseLocal.getJumlahChecked();
		urlEdit = this.url;
	  if(errMsg == ''){
	    $.ajax({
	      type:'POST',
	     ...
	     bandingDatabaseLocal.errorAlert(errMsg);
	  }
	},
	ManageDisk: function(){
	  var errMsg = bandingDatabaseLocal.getJumlahChecked();
  ...
  if(errMsg == '' || errMsg=='Pilih hanya satu data'){
    swal({
        ...
        function(){
          $.ajax({
              type:'POST',
							data : $("#"+bandingDatabaseLocal.formName).serialize(),
				      url: bandingDatabaseLocal.url+'&API=Hapus',
              success: function(data) {
                var resp = eval('(' + data + ')');
                if(resp.err==''){
                  bandingDatabaseLocal.suksesAlert("Data Terhapus",bandingDatabaseLocal.homePage);
                }else{
                  bandingDatabaseLocal.errorAlert(resp.err);
                }
              }
	...
	homePage: function(){
		window.location = bandingDatabaseLocal.url;
	},
					...
					url: this.url+'&API=prosesCheckStruktur',
						...
						var resp = eval('(' + data + ')');
							if(resp.content.sukses!=''){
								...
								$("#tempatButtonPush").html("<button type='button' onclick=bandingDatabaseLocal.pushStrukturSelected(); class='btn btn-alt btn-hover btn-success'> <span>Push</span><i class='glyph-icon icon-save'></i></button>")
								alert("Check Selesai");
							}else{
								...
								cell2.innerHTML = "<input class='custom-checkbox' type='checkbox' id='bandingDatabaseLocal_cb"+ idCheckbox+"' name='bandingDatabaseLocal_cb[]' value='"+resp.content.valueOfCheckbox+"' onchange='bandingDatabaseLocal.thisChecked('bandingDatabaseLocal_cb"+idCheckbox+"','bandingDatabaseLocal_jmlcek');'>";
								...
								$("#bandingDatabaseLocal_toogle").attr("onclick","bandingDatabaseLocal.checkSemua("+nomorUrut+",'bandingDatabaseLocal_cb','','bandingDatabaseLocal_jmlcek',this)");
					...
					url: this.url+'&API=prosesCheckTriger',
						...
						var resp = eval('(' + data + ')');
							if(resp.content.sukses!=''){
								...
								$("#prosesBarWarna").attr("style","width:"+100+"%");
								$("#prosesBarText").text("100%");
								$("#tempatButtonPush").html("<button type='button' onclick=bandingDatabaseLocal.pushTrigerSelected(); class='btn btn-alt btn-hover btn-success'> <span>Push</span><i class='glyph-icon icon-save'></i></button>")
								alert("Check Selesai");
							}else{
								$("#prosesBarValue").attr("data-value",resp.content.persen);
								$("#prosesBarWarna").attr("style","width:"+resp.content.persen+"%");
					...
					url: this.url+'&API=prosesCheckRoutine',
						...
						var resp = eval('(' + data + ')');
							if(resp.content.sukses!=''){
								...
								$("#prosesBarWarna").attr("style","width:"+100+"%");
								$("#prosesBarText").text("100%");
								$("#tempatButtonPush").html("<button type='button' onclick=bandingDatabaseLocal.pushRoutineSelected(); class='btn btn-alt btn-hover btn-success'> <span>Push</span><i class='glyph-icon icon-save'></i></button>")
								alert("Check Selesai");
							}else{
								$("#prosesBarValue").attr("data-value",resp.content.persen);
								$("#prosesBarWarna").attr("style","width:"+resp.content.persen+"%");
        ...
        url: this.url+'&API=prosesCheck',
          ...
          var resp = eval('(' + data + ')');
            if(resp.content.sukses!=''){
							...
							$("#prosesBarWarna").attr("style","width:"+100+"%");
							$("#prosesBarText").text("100%");
							$("#tempatButtonPush").html("<button type='button' onclick=bandingDatabaseLocal.pushFileSelected(); class='btn btn-alt btn-hover btn-success'> <span>Push</span><i class='glyph-icon icon-save'></i></button>")
							alert("Check Selesai");
            }else{
							$("#prosesBarValue").attr("data-value",resp.content.persen);
							$("#prosesBarWarna").attr("style","width:"+resp.content.persen+"%");
							...
							var idCheckbox = nomorUrut - 1;
							cell1.innerHTML = nomorUrut;
							cell2.innerHTML = "<input class='custom-checkbox' type='checkbox' id='bandingDatabaseLocal_cb"+ idCheckbox+"' name='bandingDatabaseLocal_cb[]' value='"+resp.content.namaFile+"' onchange='bandingDatabaseLocal.thisChecked('bandingDatabaseLocal_cb"+idCheckbox+"','bandingDatabaseLocal_jmlcek');'>";
	      ...
	      $.ajax({
			...
			optionChanged: function(){
				...
				if($("#optionBanding").val() == 'struktur'){
					$("#buttonShow").attr("onclick","bandingDatabaseLocal.showListStruktur()");
					$("#buttonCheck").attr("onclick","bandingDatabaseLocal.checkStruktur()");
					$.ajax({
		        type:'POST',
		        data : $("#"+this.formName+"_new").serialize(),
		        url: me.url+'&API=tableResultStruktur',
		          ...
		          var resp = eval('(' + data + ')');
					...
					$("#buttonShow").attr("onclick","bandingDatabaseLocal.showListTriger()");
					$("#buttonCheck").attr("onclick","bandingDatabaseLocal.checkTriger()");
					$.ajax({
		        type:'POST',
		        data : $("#"+this.formName+"_new").serialize(),
		        url: me.url+'&API=tableResultTriger',
		          ...
		          var resp = eval('(' + data + ')');
					...
					$("#buttonShow").attr("onclick","bandingDatabaseLocal.showListRoutine()");
					$("#buttonCheck").attr("onclick","bandingDatabaseLocal.checkRoutine()");
					$.ajax({
		        type:'POST',
		        data : $("#"+this.formName+"_new").serialize(),
